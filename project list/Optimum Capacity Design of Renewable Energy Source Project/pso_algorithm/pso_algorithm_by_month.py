...
from electronic_database_by_month import return_electronic_data_by_month
from weather_database_by_month import return_weather_data_by_month
...
def Optimal_PSO_Algorithm(Ef_inv, Con_LOLP, Con_dummy, prompt3, input_p_wind, input_p_pv, input_p_load):
    ...
    for prompt2 in np.arange(1, 21, 0.25):
        for prompt1 in np.arange(1, 21, 0.25):
...
def get_renewable_energy():
    ...
    # 재생 에너지 데이터 불러오기
    weather_df = return_weather_data_by_month()
    electronic_df = return_electronic_data_by_month()
