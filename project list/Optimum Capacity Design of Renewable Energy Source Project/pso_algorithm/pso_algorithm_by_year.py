...
from electronic_database_by_year import return_electronic_data_by_year
from weather_database_by_year import return_weather_data_by_year
...
def Optimal_PSO_Algorithm(Ef_inv, Con_LOLP, Con_dummy, prompt3, input_p_wind, input_p_pv, input_p_load):
    ...
    for prompt2 in np.arange(1, 31, 0.01):
        for prompt1 in np.arange(1, 31, 0.01):
...
def get_renewable_energy():
    ...
    # 재생 에너지 데이터 불러오기
    weather_df = return_weather_data_by_year()
    electronic_df = return_electronic_data_by_year()
    ...
    Con_LOLP = 0.05
    Con_dummy = 0.04
    prompt3 = 16 # 최대 부하값으로 설정해주기(지역마다)
