def Function(Ef_inv, Con_LOLP, Con_dummy, prompt3, flag):
    prompt1 = 1  # prompt1은 1~20까지
    prompt2 = 1  # prompt2는 1~20까지
    ...
    while True:
        ...
        # Types of Power Pattern generated by Photovotaic during a one day(5~25)
        ...
        # P_wind와 P_pv 값이 20이 넘어가면 더 이상 Optimal 값을 찾을 수 없으므로 탈출
        if prompt1 == 20 and prompt2 == 20:
            print('optimal no exist')
            break

        # 한 턴에서 Optimal 값을 찾지 못할 경우 prompt2부터 1씩 증가
        if flag == False:
            prompt2 += 1

        # prompt2가 21이 될 때까지 값을 찾지 못할 경우 prompt1을 1증가, prompt2는 1로 다시 초기화
        if flag == False and prompt2 == 21:
            prompt2 = 1
            prompt1 += 1