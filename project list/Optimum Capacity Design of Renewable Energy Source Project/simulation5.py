...
def Function2(Ef_inv, Con_LOLP, Con_dummy, prompt1, prompt2, prompt3):
    ...
    P_wind = [0.81, 1.00, 0.57, 0.38, 0.22, 0.35, 0.44, 0.36, 0.57, 0.82, 0.58, 0.33, 0.35, 0.64, 0.64, 0.22, 0.18,
              0.10, 0.05, 0.08, 0.15, 0.08, 0.14, 0.13]
...
for prompt1, prompt2 in zip(prompt1_result, prompt2_result):
    IC = 1.4 * 3000 * prompt1 + 1.2 * 2290 * prompt2 * round(prompt2 / 1) + C_bat * 213 + P_dgr * 850
    if min_cost >= IC:
        min_cost = IC
        min_prompt1 = prompt1
        min_prompt2 = prompt2
...
# SOC 그래프
plt.subplot(4, 2, 7)
y_pos6 = [round(P_dg[idx], 3) for idx, y in enumerate(P_dg)]
plt.plot(x_pos, y_pos6)
plt.xlabel('t(h)')
