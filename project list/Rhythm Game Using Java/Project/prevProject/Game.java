...
public class Game extends Thread { // 하나의 게임은 하나의 단위로 움직여야 되므로 클래스를 생성
	...
	private Image noteRouteLineImage = new ImageIcon(Main.class.getResource("../images/noteRouteLine.jpg")).getImage(); // noteRouteLineImage 변수 생성
	private Image judgementLineImage = new ImageIcon(Main.class.getResource("../images/judgementLine.jpg")).getImage(); // judgementLineImage 변수 생성
	private Image gameInfoImage = new ImageIcon(Main.class.getResource("../images/gameInfo.jpg")).getImage(); // gameInfoImage 변수 생성

	private Image noteRouteSImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성	
	private Image noteRouteDImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	private Image noteRouteFImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	private Image noteRouteSpace1Image = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	private Image noteRouteSpace2Image = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	private Image noteRouteJImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	private Image noteRouteKImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	private Image noteRouteLImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage(); // noteRouteImage 변수 생성
	...
	public Game(String t, String d, String m) {
		...
		gMusic = new Music(this.mTitle, false);
	...
	public void screenDraw(Graphics2D g) {
		...
		// noteList에 있는 노트들을 하나씩 불러와 그려준다
		for (int i = 0; i < noteList.size(); i++) {
			Note note = noteList.get(i);
			
			if (!note.isProceeded()) { // 사용되지 않는 노트는 사라짐
				noteList.remove(i);
				i--;
			} else {
				note.screenDraw(g);
	...
	public void pressS() { // 키보드 S를 눌렀을 때 이벤트 처리
		
		judge("S"); // S에 대한 판정
		
		noteRouteSImage = new ImageIcon(Main.class.getResource("../images/noteRoutePressed.jpg")).getImage();
		/*
		 *  만약 키보드를 눌렀을 때 효과음을 살리고 싶다면
		 *  new Music("음악 이름.mp3", false).start();
		 */
	}
	
	public void releaseS() { // 키보드 S를 눌렀다가 뗐을 때 이벤트 처리
		noteRouteSImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage();
	}
	
	public void pressD() { // 키보드 D를 눌렀을 때 이벤트 처리
	...
	public void releaseD() { // 키보드 D를 눌렀다가 뗐을 때 이벤트 처리
		noteRouteDImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage();
	}
	
	public void pressF() { // 키보드 F를 눌렀을 때 이벤트 처리
	...
	public void releaseF() { // 키보드 F를 눌렀다가 뗐을 때 이벤트 처리
		noteRouteFImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage();
	}
	
	public void pressSpace() { // 키보드 Space를 눌렀을 때 이벤트 처리
	...
	public void releaseSpace() { // 키보드 Space를 눌렀다가 뗐을 때 이벤트 처리
	...
	public void pressJ() { // 키보드 J를 눌렀을 때 이벤트 처리
	...
	public void releaseJ() { // 키보드 J를 눌렀다가 뗐을 때 이벤트 처리
		noteRouteJImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage();
	}
	
	public void pressK() { // 키보드 K를 눌렀을 때 이벤트 처리
	...
	public void releaseK() { // 키보드 K를 눌렀다가 뗐을 때 이벤트 처리
		noteRouteKImage = new ImageIcon(Main.class.getResource("../images/noteRoute.jpg")).getImage();
	}
	
	public void pressL() { // 키보드 L를 눌렀을 때 이벤트 처리
	...
	public void releaseL() { // 키보드 L를 눌렀다가 뗐을 때 이벤트 처리
	...
	public void close() {
		gMusic.close(); // 게임 창 뒤로 가기를 하였을 때 곡이 재생되는 것을 막는다
		this.interrupt();
	}
	...
	public void dropNote(String titleName) {
		...
		int i = 0;
		gMusic.start(); // 여기다 넣어주어야 비트들이 모두 생성되는 시간과 비트의 시작의 격차를 줄일 수 있다
		...
		while (i < beats.length && !isInterrupted()) { // beats 배열에서 얻은 이름을 이용해 노트들을 하나씩 생성해주며 시작하게 한다
			...
			if (beats[i].getTime() <= gMusic.getTime()) {
